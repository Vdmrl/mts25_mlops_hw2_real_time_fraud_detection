...
1. **`interface`** (Streamlit UI):
   ...
   Создан для удобной симуляции потоковых данных с транзакциями. Реальный продукт использовал бы прямой поток данных из других систем.
    ...
    - Извлекает результаты из базы данных PostgreSQL
    - отображает последние мошеннические транзакции
    - отображает распределение скоров
...
3. **`score_ingester`** (Database service)
    - Получает (consume) результаты скоринга из топика kafka `scoring`
    - Выгружает результаты скоринга в базу данных PostgreSQL
...
# Для упрощения запуска не добавлял .env, параметры базы данных можно изменить только в самом docker-compose
